...
export function WorkSummaryTable({ summary }: { summary: WorkSummary }) {
    ...
    const workedPercent =
        workspace.workProgressBasis === 'amount'
            ? summary.totalAmount > 0
                ? Math.min((summary.workedAmount / summary.totalAmount) * 100, 100)
                : 0
            : summary.totalCount > 0
              ? Math.min((summary.workedCount / summary.totalCount) * 100, 100)
              : 0;

    return (
        <div className="border-muted/70 bg-card overflow-hidden rounded-md border">
            <div
                className={`border-border bg-primary/10 grid ${workspace.showTrueBalance ? 'grid-cols-[minmax(0,1.2fr)_minmax(90px,0.8fr)_minmax(130px,1fr)]' : 'grid-cols-[minmax(0,1.2fr)_minmax(90px,0.8fr)]'} border-b text-sm font-semibold`}
            >
            ...
            <div
                className={`border-border bg-muted/20 grid ${workspace.showTrueBalance ? 'grid-cols-[minmax(0,1.2fr)_minmax(90px,0.8fr)_minmax(130px,1fr)]' : 'grid-cols-[minmax(0,1.2fr)_minmax(90px,0.8fr)]'} border-b text-sm font-semibold`}
            >
                ...
                {workspace.showTrueBalance && (
                    <div className="border-border text-primary border-l px-3 py-2 text-right tabular-nums">
                        {formatCurrency(summary.workedAmount)}
                    </div>
                )}
            </div>
            <div
                className={`bg-muted/10 grid ${workspace.showTrueBalance ? 'grid-cols-[minmax(0,1.2fr)_minmax(90px,0.8fr)_minmax(130px,1fr)]' : 'grid-cols-[minmax(0,1.2fr)_minmax(90px,0.8fr)]'} text-sm font-semibold`}
            >
